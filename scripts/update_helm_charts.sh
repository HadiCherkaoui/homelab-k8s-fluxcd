...
# When running in CI, fail fast if push credentials are missing so we don't
# spend ~30s detecting updates only to fail at `git push` with HTTP 401.
if [[ -n "${CI:-}" && -z "${DEPLOY_TOKEN:-}" ]]; then
	echo "DEPLOY_TOKEN is not set; cannot push branch or open MR" >&2
	echo "Add it as a masked CI/CD variable on the project (Project Access Token with write_repository)" >&2
	exit 1
fi
